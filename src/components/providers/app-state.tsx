...
import type { AgentMode, Approval, ApprovalStatus, Integration, Location, Settings } from "@/core/types";
...
export function AppStateProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    let alive = true;
    Promise.all([adapters.sales.listLocations(), adapters.sales.latestDate(DEFAULT_LOCATION), adapters.agents.listAgents()]).then(([locs, date, agents]) => {
      if (!alive) return;
      setLocations(locs);
      setAsOf(date);
  ...
  useEffect(() => {
    ...
    adapters.approvals.listApprovals(locationId).then((rows) => {
  ...
  useEffect(() => {
    ...
    inflight.current.add(key);
    adapters.integrations.listIntegrations(locationId).then((rows) => {
      inflight.current.delete(key);
