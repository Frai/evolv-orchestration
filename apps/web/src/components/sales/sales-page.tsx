...
import { deadItems, hourlyHeatmap, itemTotals, sumBy, topItems } from "@evolv/contracts/sales";
import { capacityFor, kitchenSeverity, peakKitchenLoad, peakWindowLabel } from "@evolv/contracts/kitchen";
import { int, money, one, pct, signedPct } from "@evolv/contracts/format";
...
export function SalesPage() {
  ...
  const sales = useAsync(() => apiClient.sales.getSalesDays({ locationId, outletId, range }), [locationId, outletId, asOf, win]);
  const items = useAsync(async () => {
    const [rows, menu] = await Promise.all([apiClient.sales.getItemSales({ locationId, outletId, range }), apiClient.sales.getMenu(locationId, outletId)]);
